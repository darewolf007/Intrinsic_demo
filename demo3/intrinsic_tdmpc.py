...
class Intrinsic_TDMPC2(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.optim = torch.optim.Adam(
            [
                ...
                {"params": self.model._dynamics_ensemble.parameters()},  # [ENSEMBLE] 修改为ensemble
        ...
        # [ENSEMBLE] Bootstrap训练参数
        self.bootstrap_prob = getattr(cfg, 'bootstrap_prob', 0.8)
        
        # [ENSEMBLE] 不确定性惩罚参数
        self.uncertainty_penalty_max = getattr(cfg, 'uncertainty_penalty_max', 1.0)
        self.uncertainty_bonus_max = getattr(cfg, 'uncertainty_bonus_max', 0.3)
        self.disc_loss_threshold_high = getattr(cfg, 'disc_loss_threshold_high', 0.25)
        self.disc_loss_threshold_low = getattr(cfg, 'disc_loss_threshold_low', 0.05)
        
        # [ENSEMBLE] 用于存储当前disc_loss，由trainer传入
        self._current_disc_loss = 0.3  # 默认值
        
        # [ENSEMBLE] 不确定性归一化器
        self.uncertainty_normalizer = RunningMeanStd(shape=(1,), epsilon=1e-4).to(self.device)
    ...
    def _estimate_value(self, z, actions, task):
        G, discount = 0, 1
        total_uncertainty = 0  # [ENSEMBLE] 累积不确定性
        ...
        for t in range(self.cfg.horizon):
            ...
            # [ENSEMBLE] 使用带不确定性的预测
            z_next, uncertainty, _ = self.model.next_with_uncertainty(z, actions[:, t], task)
            total_uncertainty = total_uncertainty + discount * uncertainty.unsqueeze(-1)
            z = z_next
        ...
        terminal_value = self.model.Q(
            z, self.model.pi(z, task)[1], task, return_type="avg"
        )
        
        # [ENSEMBLE] 应用不确定性调整
        uncertainty_coef = self._get_uncertainty_coef()
        # 归一化不确定性
        if total_uncertainty.numel() > 0:
            self.uncertainty_normalizer.update(total_uncertainty.reshape(-1, 1))
        norm_uncertainty = self.uncertainty_normalizer(total_uncertainty)
        
        # uncertainty_coef > 0: bonus (探索)
        # uncertainty_coef < 0: penalty (保守)
        uncertainty_adjustment = uncertainty_coef * norm_uncertainty
        
        return G + discount * terminal_value + uncertainty_adjustment
    ...
    def _get_uncertainty_coef(self):
        """
        [ENSEMBLE] 根据disc_loss自适应计算不确定性系数
        - disc_loss高 (早期): 返回正值 (bonus，鼓励探索)
        - disc_loss低 (后期): 返回负值 (penalty，保守优化)
        """
        disc_loss = self._current_disc_loss
        
        if disc_loss > self.disc_loss_threshold_high:
            # 早期：探索bonus
            return self.uncertainty_bonus_max
        elif disc_loss < self.disc_loss_threshold_low:
            # 后期：保守penalty
            return -self.uncertainty_penalty_max
        else:
            # 过渡期：线性插值
            progress = (self.disc_loss_threshold_high - disc_loss) / \
                       (self.disc_loss_threshold_high - self.disc_loss_threshold_low)
            return self.uncertainty_bonus_max - \
    ...
    def _update(
        ...
        for t, (_action, _next_z) in enumerate(zip(action.unbind(0), next_z.unbind(0))):
            # [ENSEMBLE] Bootstrap训练：每个ensemble成员在随机子集上训练
            if update_dynamics:
                batch_size = _action.shape[0]
                for ens_idx in range(self.model.num_ensemble):
                    # 生成bootstrap mask
                    mask = torch.rand(batch_size, device=self.device) < self.bootstrap_prob
                    if mask.sum() < 4:  # 确保至少有一些样本
                        mask[:4] = True
                    
                    # 只用选中的样本计算这个ensemble的loss
                    z_pred_ens = self.model.next(z[mask], _action[mask], task, ensemble_idx=ens_idx)
                    ens_loss = F.mse_loss(z_pred_ens, _next_z[mask])
                    consistency_loss = consistency_loss + ens_loss * self.cfg.rho**t / self.model.num_ensemble
